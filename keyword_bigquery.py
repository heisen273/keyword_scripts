...
for f in files:
	...
	os.system('bq mk KeywordCollections.'+f.split('_')[1].split('.')[0]+'_Keywords  KeywordName:string,URL:string,IsKeywordDeleted:string,LastCrawlingDate:string,LastCategoryId:string,LastKeywordName:string,NoIndexNofollowMetaTag:string,TotalSearch:string,i301:string,Has301:string,QualityFactor:string,AMS:string')
	if f == 'Manipulated_FR.csv':
		s = subprocess.Popen(['bq','load','--max_bad_records=10','--field_delimiter=|','KeywordCollections.'+f.split('_')[1].split('.')[0]+'_Keywords', './Manipulated_bpfs-fr2.csv'], stdout=subprocess.PIPE,stderr=subprocess.PIPE)
	else:
		s = subprocess.Popen(['bq','load','--max_bad_records=10','--field_delimiter=|','KeywordCollections.'+f.split('_')[1].split('.')[0]+'_Keywords', './'+f], stdout=subprocess.PIPE,stderr=subprocess.PIPE)
